...
function parseWorkbook(dir) {
  ...
  // Extract sheets: <sheet ... name="..." sheetId="1" .../>
...
const ijabDir = path.join(rootDir, 'data', '_tmp_ijab');
const omahDir = path.join(rootDir, 'data', '_tmp_omah');

console.log('Parsing Ijab...');
const ijabData = parseWorkbook(ijabDir);
fs.writeFileSync(path.join(rootDir, 'data', 'ijab_parsed.json'), JSON.stringify(ijabData, null, 2), 'utf8');
console.log('Ijab parsed successfully, sheets:', Object.keys(ijabData));

console.log('Parsing Omah...');
const omahData = parseWorkbook(omahDir);
fs.writeFileSync(path.join(rootDir, 'data', 'omah_parsed.json'), JSON.stringify(omahData, null, 2), 'utf8');
console.log('Omah parsed successfully, sheets:', Object.keys(omahData));
